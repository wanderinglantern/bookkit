...
IMPLEMENTED: dict[str, str] = {
    ...
    "new_submission": (
        "BUILT 2026-08-19 (phase 2), REPLACED 2026-08-26. It was GET/POST "
        ".../program/{placement_id}/submissions rendering the shared "
        "submission_form into the program section's form host — and what it "
        "wrote was a package with NO LINE OF COVERAGE and no market response, "
        "which is the second of the two controls that both meant 'we sent this "
        "market a submission' (A4). It is the one that manufactured the defect: "
        "fourteen seeded placements whose Marketing panel said 'No line of "
        "coverage on this placement is being marketed yet' over live "
        "submissions, two of them quoted at $1.4M, and a client workbook with "
        "one header row. Both routes are gone. The band's Submission button is "
        "an anchor to the Marketing section, where the add-market row "
        "(POST .../marketing/lines/{line_id}/approaches) records the same "
        "approach against the line of coverage it is about, through the one "
        "home for that write (services.marketing_entry.approach, shared with "
        "MCP's market_approach) — strictly more than the old form recorded, "
        "except for the underwriter and the package note, which that row has "
        "no field for yet and which are named as the cost in "
        "routes/program.py. Marketing already recorded with no line is not "
        "stranded: it renders in the report's provisional block and each row "
        "is given its line at POST .../marketing/submissions/{id}/line. "
        "(The 2026-08-19 audit caught this entry still PENDING after the "
        "build — the ledger's prose is hand-maintained, and a shipped feature "
        "must move its entry in the same commit.)"
    ),
    "renew_placement": (
