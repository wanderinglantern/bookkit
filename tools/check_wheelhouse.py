...
re-uploads it. That drill gets skipped — towerkit's openpyxl landed on
2026-08-11 and its asset stayed stale until an ``OFFLINE=1`` install failed on
a machine with no PyPI access, which is exactly the machine that cannot route
around it.

bookkit inherits that risk twice over: install.sh builds towerkit editable
from ../towerkit, so this wheelhouse must satisfy towerkit's dependencies too,
and those change in a repo no push here can see. CI passes both pyprojects.
