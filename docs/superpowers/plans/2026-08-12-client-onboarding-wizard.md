...
4. Route the existing per-tab verbs: in the `action_add` tab dispatch, `tab-open-items` → same task_form flow as overview (org pre-attached); `e`/`d` operate on the datasheet's selected task via the same `_selected_key` pattern the overview uses (read :828 and mirror). Add `Binding("x", "export_open_items", "Export open items")` SCREEN-WIDE on AccountScreen (Grant's call 2026-08-13: export must be reachable from the whole client view, not just this tab) → `action_export_open_items` calls `entity_actions.export_open_items_flow(self, self.org_id)` from any tab. Keep `x` in the open-items TAB_HINTS entry; add it to the other tabs' hint strings only if they have room — the footer carries it everywhere regardless.
